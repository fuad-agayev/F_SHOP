...
<template>
  <nav 
    class="fixed top-0 left-0 right-0 bg-white z-50 transition-all duration-300"
    :class="{
      'bg-white/80 backdrop-blur-lg shadow-lg dark:bg-gray-800/80': isScrolled,
      ...
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="relative hidden md:block">
            <button
              ...
              class="flex items-center space-x-1 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white categories-button"
            >
              <Bars3Icon class="w-5 h-5" />
              <span>Categories</span>
              <ChevronDownIcon class="w-4 h-4" :class="{ 'transform rotate-180': showCategoriesMenu }" />
            </button>

            <!-- Categories Menu -->
            <div v-if="showCategoriesMenu" 
                 class="absolute left-0 top-full mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg z-50 categories-menu"
                 @click.stop>
              <!-- Desktop View -->
              <div class="grid grid-cols-4 divide-x dark:divide-gray-700">
                    ...
                    <div v-for="category in categories" 
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <ChevronRightIcon 
                          ...
                          :class="{ 'text-blue-500': hoveredCategory?.id === category.id }"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <div class="p-4">
                  ...
                  <div v-if="hoveredCategory?.subcategories" class="space-y-2">
                    <div v-for="subcategory in hoveredCategory.subcategories" 
                         :key="subcategory.id"
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <ChevronRightIcon 
                          v-if="hasNestedSubcategories(subcategory)"
                          class="w-4 h-4 text-gray-400"
                ...
                <div class="p-4">
                  <h3 class="font-semibold text-gray-900 dark:text-gray-100 mb-4">More Categories</h3>
                  <div v-if="hoveredSubcategory?.nested_subcategories" class="space-y-2">
                    <div v-for="nested in hoveredSubcategory.nested_subcategories" 
                         :key="nested.id"
                         @mouseenter="hoveredNestedSubcategory = nested"
                         class="cursor-pointer p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700"
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <ChevronRightIcon 
                          v-if="hasDeepSubcategories(nested)"
                          class="w-4 h-4 text-gray-400"
                          :class="{ 'text-blue-500': hoveredNestedSubcategory?.id === nested.id }"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <div class="p-4">
                  ...
                  <div v-if="hoveredNestedSubcategory?.deep_subcategories" class="space-y-2">
                    <div v-for="deep in hoveredNestedSubcategory.deep_subcategories" 
                         :key="deep.id"
                         class="cursor-pointer p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
                      <RouterLink 
                        :to="`/category/${deep.id}`"
                        class="text-gray-500 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400"
                        @click="closeMenu"
                      >
                        {{ deep.name }}
                      </RouterLink>
        ...
        <div class="flex-1 max-w-3xl mx-4 hidden md:block">
          <form @submit="handleSearch" class="relative search-container">
            ...
            <div v-if="showSearchResults && searchResults.length > 0"
                 class="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden z-50">
              <div class="max-h-96 overflow-y-auto">
                <RouterLink v-for="product in searchResults"
                           :key="product.id"
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="relative dark-mode-container">
            <button
              @click="showDarkModeMenu = !showDarkModeMenu"
              class="p-2 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white"
            >
            ...
            <div v-if="showDarkModeMenu"
              ...
              <div class="p-2">
                ...
                <button
                  @click="setDarkMode(true); showDarkModeMenu = false"
                  class="flex items-center w-full px-4 py-2 text-left rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700"
                  :class="{ 'bg-gray-100 dark:bg-gray-700': isDark }"
                >
          ...
          <RouterLink
            to="/cart"
            class="relative p-2 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white"
          >
          ...
          <div v-if="isAuthenticated" class="relative">
            ...
            <button 
              @click="logout"
              class="ml-4 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white"
            >
              Logout
            </button>
          </div>
      ...
      <div v-if="showMobileMenu" class="md:hidden">
        <div class="px-2 pt-2 pb-3 space-y-1">
          ...
          <form @submit="handleSearch" class="mb-4">
            ...
            <div v-if="showSearchResults && searchResults.length > 0"
                 class="mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
              <div class="max-h-96 overflow-y-auto">
                <RouterLink v-for="product in searchResults"
                           :key="product.id"
                           :to="`/product/${product.id}`"
          ...
          <div class="mb-4">
            ...
            <div v-else-if="!mobileSelectedSubcategory" class="space-y-2">
              ...
              <div v-for="subcategory in mobileSelectedCategory.subcategories" 
                   :key="subcategory.id"
                   class="rounded-lg overflow-hidden">
                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 cursor-pointer"
                     @click="hasNestedSubcategories(subcategory) ? selectMobileSubcategory(subcategory) : null">
                  <RouterLink 
                    :to="`/category/${subcategory.id}`"
                    class="flex-grow text-gray-800 dark:text-gray-200"
                    @click="showMobileMenu = false"
                  ...
                  <ChevronRightIcon v-if="hasNestedSubcategories(subcategory)" class="w-5 h-5 text-gray-400" />
                </div>
              </div>
            </div>
            ...
            <div v-else-if="!mobileSelectedNestedSubcategory" class="space-y-2">
              <h3 class="font-medium text-gray-800 dark:text-gray-200 mb-4">
                {{ mobileSelectedSubcategory.name }}
              </h3>
              <div v-for="nested in mobileSelectedSubcategory.nested_subcategories" 
                   :key="nested.id"
                   class="rounded-lg overflow-hidden">
                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 cursor-pointer"
            ...
            <div v-else class="space-y-2">
              <h3 class="font-medium text-gray-800 dark:text-gray-200 mb-4">
                {{ mobileSelectedNestedSubcategory.name }}
              </h3>
              <div v-for="deep in mobileSelectedNestedSubcategory.deep_subcategories" 
                   :key="deep.id"
                   class="rounded-lg overflow-hidden">
                <RouterLink 
                  :to="`/category/${deep.id}`"
                  class="block p-3 bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-200"
                  @click="showMobileMenu = false"
                >
                  {{ deep.name }}
                </RouterLink>
